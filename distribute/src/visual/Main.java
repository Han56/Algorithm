...
//		});
	...
	public static void main(String[] args) throws NumberFormatException, IOException, ParseException {
		...
		//根据情景模式选择不同的运行代码。
		if(MainTest.runningModel == MainTest.REMOTE_ONLINE_STORAGE || MainTest.runningModel == MainTest.REMOTE_ONLINE_UNSTORAGE) {
			new RunningSceneConfig(MainTest.runningModel);
			printRunningParameters.printAllParameters();
		}
		else if(MainTest.runningModel == MainTest.LOCAL_OFFLINE_STORAGE || MainTest.runningModel == MainTest.LOCAL_OFFLINE_UNSTORAGE) {
			new RunningSceneConfig(MainTest.runningModel);
			InitialConfig config = new InitialConfig(MainTest.prePath);
		}
		else if(MainTest.runningModel == MainTest.checkModel) {
			new RunningSceneConfig(MainTest.runningModel);
			MainTest.is_delay = 1;
			InitialConfig config = new InitialConfig(MainTest.prePath);
		}
